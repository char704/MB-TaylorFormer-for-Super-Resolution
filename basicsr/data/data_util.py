...
def paired_paths_from_folder(folders, keys, filename_tmpl,phase='test'):
    ...
    for idx in range(len(input_paths)):
        ...
        #print(basename,ext)
        if phase == 'train':
            if 'ITS' or 'Haze4K' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0].split('_')[0]+ext)
            elif 'OTS' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0].split('_')[0]+ext)
            elif 'NH_Haze' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0]+'_GT'+ext)
            elif 'Dense-Haze' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0]+'_GT'+ext)
            elif 'O-HAZE' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_hazy')[0]+'_GT'+ext)
            elif 'I-HAZY' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0]+'_indoor_GT'+ext)
            elif 'SRRS' in gt_folder:
                gt_path = osp.join(gt_folder,basename+'.jpg')
            else:
                gt_path = osp.join(gt_folder,basename.split('_')[0]+ext)
            
        else:
            if 'Haze4K' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0].split('_')[0]+ext)
            if 'ITS' in gt_folder:
                gt_path = osp.join(gt_folder,basename+ext)
            elif 'OTS' in gt_folder:
                gt_path = osp.join(gt_folder,basename+ext)
            elif 'NH_Haze' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0]+'_GT'+ext)
                ...
                gt_path = osp.join(gt_folder,basename.split('_haze')[0]+'_GT'+ext)#.split('_')[0]+'_outdoor_GT'+ext)
            elif 'I-HAZY' in gt_folder:
                gt_path = osp.join(gt_folder,basename.split('_')[0]+'_indoor_GT'+ext)
            elif 'SRRS' in gt_folder:
                gt_path = osp.join(gt_folder,basename+'.jpg')
